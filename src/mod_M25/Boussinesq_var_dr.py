...
def solve(dr_scale):
    ...
    save_name = dir_name + f'/freq-s-dr{dr_scale:.2f}-r{Ro:.3f}_{args.m}x{Ntheta}x{Nr}'
...
if __name__ == "__main__":
    # pass
    # dr_arr = np.linspace(0., 1., num=51)
    dr_arr = 0.01*np.r_[np.arange(0, 30, 2), np.arange(30, 70, 4), np.arange(70, 101, 6)]
    # dr_arr = np.array([0.01, 0.02, 0.03, 0.04, 0.06, 0.08, 0.10, 0.12, 0.11])
    print(f"{dr_arr.size} rotation steps to be computed...", flush=True)
    for dr_scale in dr_arr:
        solve(dr_scale)
