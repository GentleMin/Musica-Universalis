...
def main():
    ...
    # A, B = setup_op_spin(M_lib, Ek, Em, Le)
    # fname = os.path.join(args.dest, f'spec_Ek{Ek:.2e}_Em{Em:.2e}_Le{Le:.2e}')
    
    if args.symmv is not None and args.symmb is not None:
        # print(4*(args.N+1)*(args.dL+1), A.shape)
        A = sym_slicer.slice_sym_MHD(A, args.N+1, args.dL+1, sym_v=args.symmv, sym_b=args.symmb)
        B = sym_slicer.slice_sym_MHD(B, args.N+1, args.dL+1, sym_v=args.symmv, sym_b=args.symmb)
