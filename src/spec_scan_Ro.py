...
def main_scanRo_Hydro():
    ...
    for i_par, par in enumerate(par_list):
        ...
        print(f"\n------------------------------ Ro = {par:+.3e} ---------------------------------------------")
        Ek = args.Ek
        A, B = setup_op_spin_DR(M_lib, E=Ek, Em=0, Le=0, Ro=par)
        fname = os.path.join(args.dest, f'spec_Ek{Ek:.2e}_Ro{par:.2f}')
...
def main_scanRo_MHD():
    ...
    for i_par, par in enumerate(par_list):
        ...
        print(f"\n------------------------------ Ro = {par:+.3e} ---------------------------------------------")
        Ek, Em, Le = args.Ek, args.Em, args.Le
        A, B = setup_op_spin_DR(M_lib, E=Ek, Em=Em, Le=Le, Ro=par)
        fname = os.path.join(args.dest, f'spec_Ek{Ek:.2e}_Em{Em:.2e}_Le{Le:.2e}_Ro{par:.2f}')
        ...
        if args.hydro:
            if args.symmv is not None:
                ...
                N = args.N if args.dedalus_operators else args.N + 1
                dL = args.dL if args.dedalus_operators else args.dL + 1
                A, P = sym_slicer.slice_Hydro_sym_from_MHD(A, N, dL, sym_v=args.symmv, perm=perm)
                B, _ = sym_slicer.slice_Hydro_sym_from_MHD(B, N, dL, sym_v=args.symmv, perm=perm)
                fname = fname + f'_v{args.symmv}_hydro' 
            else:
                perm = M_lib['perm'] if 'perm' in M_lib else None
                N = args.N if args.dedalus_operators else args.N + 1
                dL = args.dL if args.dedalus_operators else args.dL + 1
                A, P = sym_slicer.slice_Hydro_from_MHD(A, N, dL, perm=perm)
                B, _ = sym_slicer.slice_Hydro_from_MHD(B, N, dL, perm=perm)
                fname = fname + "_hydro"
        else:
