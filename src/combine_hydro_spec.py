    ...
    for rootdir in root_dirs:
        ...
        for matdir in dir_list:
            ...
            for evfile in f_list:
                ...
                evtmp = np.load(fpath)
                np.save(fpath, -evtmp)
                print(f'\r\t{evfile} flipped                   ', flush=True, end='')
                time.sleep(0.02)
            print()


def rename_spectra(root_dirs):
    for rootdir in root_dirs:
        dir_list = sorted(os.listdir(rootdir))
        print(rootdir)
        for matdir in dir_list:
            ...
            for evfile in f_list:
                ...
                fpath = os.path.join(rootdir, matdir, evfile)
                evtmp = np.load(fpath)
                fpath_new = os.path.join(rootdir, matdir, 
                    f"spec_Ek5.00e-05_Em{meta_ev['Em']:.2e}_Le{meta_ev['Le']:.2e}_Ro{meta_ev['Ro']:.2f}_v{meta_ev['symv']:d}_hydro.npy")
                np.save(fpath_new, evtmp)
                print(f'\r\t{evfile} renamed', flush=True, end='')
                time.sleep(0.02)
            print()
...
if __name__ == '__main__':
    ...
    # flip_spectra(args.i)
    # rename_spectra(args.i)
    combine_spectra(args.i, args.o)
